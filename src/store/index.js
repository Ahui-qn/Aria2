...
export default new Vuex.Store({
    ...
    mutations: {
        ...
        editServer(state, {type, value, currentServerId}) {   // 修改现有服务器数据
            ...
            localStorage.setItem('aria2Servers', JSON.stringify(state.servers))   // 还需要存储
        ...
        sort(state, {currentPage, type}) {
            if (currentPage == '/completed') {
                ...
                this.dispatch('getTellStopped')   // 立刻更新数据
            }else if (currentPage == '/deleted') {
                state.discard = quickSort(state.discard, state.sort, type)
                this.dispatch('getTellStopped')
            }else {
                state.tasks = quickSort(state.tasks, state.sort, type)   // 正在下载的排序有问题
        ...
        editUpdateList(state, tasks) {   // actions触发的task更新赋值函数
            if (state.currentSort == 'size') {
                state.tasks = quickSort(tasks, state.sort, 'size')
            }else {
                state.tasks = quickSort(tasks, state.sort, 'progress')
            }
        },
        ...
        editConnectionStatus (state, status) {
    ...
    actions: {
        async switchServer({state, commit}, server) {   // 获取切换到的服务器字段
            ...
            let aria2 = new Aria2Client(server)
            commit('editAria2', aria2)
            commit('editConnectionStatus', '连接中')
            try {
                await aria2.ready()    // 用于测试websocket连接是否成功
                await aria2.tellActive()  // 用于测试Aria2是否发送成功
                commit('editConnectionStatus', '已连接')
            } catch(e) {
                commit('editConnectionStatus', '未连接')
            }
        },

        async updateList({state, commit}, intervalId) {     // Downloading组件页面每秒钟调用此函数更新数据， 接收传过来的定时器
            let tasks = []
            try {
                    tasks = [
            ...
            }catch (e) {  // 由于服务器所有错误代码都是1所以直接catch
                clearInterval(intervalId)
                if (e == 'WS_CONNECTION_ERROR') {
                    Message({
                        message: 'WebSocket未连接！ 请检查aria2是否启动或刷新页面',
                        type: 'error',
                        duration:10000
                    })
                    commit('editConnectionStatus', '未连接')
            ...
            commit('editUpdateList', tasks)
        },
        async getTellStopped({commit, state}, intervalId) {   // 分类出已下载完成的和已废弃的
            let completeds = []
            let discard = []
            let tasks = []
            try {
        ...
        async editStatus({ state }, status) {   // 操作task删除、暂停、开始的操作
            ...
            this.dispatch('updateList')    // 调用上面的update更新状态
        },
    },

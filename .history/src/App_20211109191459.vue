    ...
    height: 100%;
     .el {
        ...
        .outmost-menu {
            ...
            .success {
                background-color: rgb(101, 255, 168);
            }
            .falied {
        ...
        <div class="el">
            <el-menu class="outmost-menu" text-color="#fff" active-text-color="#ffd04b" :router="true" :default-active="currentPath()">
                ...
                <el-menu-item index="/server">
                    ...
                    <span slot="title">服务器
                        <span :class="{'connectionStatus': true, 'success': connectionStatus == '已连接', 'falied' : connectionStatus == '未连接'}">
                            {{connectionStatus}}
                        </span>
                    </span>
                </el-menu-item>
        ...
        <header>
            <nav>
                <div class="edit-status">
                    ...
                    <el-select v-model="selectedServer "  @change="switchServer(selectedServer)"  value-key="id"  placeholder="请选择服务器">
                        <el-option
...
export default {
    ...
    watch : {
        selectedServer(value) {   // 当切换服务器的时候
    ...
    created() {
        ...
        this.switchServer(this.selectedServer)
    },
    computed: {
        ...mapState(['tasks','search', 'selected', 'completeds','discard', 'servers','connectionStatus']),
        isDisabled () {
        ...
        switchSelection() {  // 根据不同页面绑定不同的数据   全选按钮的cheked绑定
            let currentPage = this.$route.path

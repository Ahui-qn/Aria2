...
<template>
    <div class="settings">
        <h1>{{nameTransform()}}</h1>
        <div class="item" v-for="(value,key) in parameter" :key="key">
            <span>{{optionDesc[key]}}</span>
            <span class="english"> {{'(' + key + ')'}}</span>
            <el-switch
                class="right"
                v-if=" value == true || value == false || value == 'true' || value == 'false' "
                :value="parameter[key]"
                @change="switchHandleChange($event, key)"
                active-color="#13ce66"
            ...
            <el-input-number 
                v-else-if="/^[0-9]+$/.test(value)"
                class="right"
                :value="parameter[key]"
                @change="numberHandleChange(value, key)"
                :min="1"    
                label="描述文字">
            </el-input-number>
            <el-input v-else  class="right" v-model="parameter[key]" placeholder="请输入内容"></el-input>
        </div>
    </div>
</template>
...
export default {
    ...
    data () {   
        return {
            // 修改的值直接保存到parameter[key]，和这个v-model
            parameter: this.params,    // 需要修改的数据保存在这以来触发 响应式更新
            optionDesc : optionDesc,
            // 根据这个对象找出响应翻译然后渲染出列表
        }
    },
    watch: {    
        params(val){    // 数据为异步传递的，第一次需要侦听来赋值
            this.parameter = val;
        }
    },
    methods: {
        switchHandleChange(e, key) {  // 传递给父组件更新数据
            console.log(e)
            this.parameter[key] = e
            console.log('switchHandleChange')
            // this.$emit('change', this.parameter)
        },
        numberHandleChange(e, key) {  // 传递给父组件更新数据
            console.log(e)
            this.parameter[key] = e
            console.log('numberHandleChange')
            // this.$emit('change', this.parameter)
        },
        nameTransform() {
...
/*  传递过来一个设置对象，然后找到对应的翻译
    如果修改了parameter就触发emit事件把parameter传给给父组件去修改设置项，此页面主要用到父子组件沟通，没有vuex

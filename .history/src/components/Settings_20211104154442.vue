<style scoped lang='less'>
    .settings {
        ...
        h3 {
            ...
            border-top: 1px rgba(198, 198, 198, 0.715) solid;
        }
        .item {
            height: 50px;
...
<template>
    <div class="settings">
        <h3>{{this.$route.params.type.toUpperCase()}}</h3>
        <div class="item" v-for="(value,key) in parameter" :key="key">
            <span>{{optionDesc[key] + '(' + key + ')'}}</span>
            <el-switch
                class="right"
            ...
            <!-- <el-input class="right" v-else v-model="parameter[key]" placeholder="请输入内容"></el-input> -->
        </div>
    </div>
</template>

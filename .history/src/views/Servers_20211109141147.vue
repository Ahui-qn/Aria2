            ...
            <el-tab-pane v-for="item in servers" :key="item.id" :label="item.name" :name="item.id">
                <el-form>
                    <el-form-item label="RPC别名" label-width="80px">
                        <el-input :value="item.name" @input.native="editNewServer($event, 'name')"></el-input>
                    </el-form-item>
                    <el-form-item label="HOST" label-width="80px">
                        <el-input :value="item.host"  @input.native="editNewServer($event, 'host')"></el-input>
                    </el-form-item>
                    <el-form-item label="端口号" label-width="80px">
                        <el-input-number :value="item.port"  @input.native="editNewServer($event, 'port')" :min="1" :max="65535" ></el-input-number>
                    </el-form-item>
                    <el-form-item label="密码" label-width="80px">
                        <el-input type="password" show-password :value="item.secret"  @input.native="editNewServer($event, 'secret')"></el-input>
                    </el-form-item>
                </el-form>
            </el-tab-pane>
        </el-tabs>
...
export default {
    ...
    methods: {
        ...mapMutations(['initServers','addServers', 'removeServers']),

        editNewServer(e, type) {  // 此函数用于修改newServer以来添加或修改服务器数据
            this.newServer[type] = e.target.value
        },

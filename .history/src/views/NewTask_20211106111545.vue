    ...
    <div class="newTask">
        <div class="downloadType">
            ...
            <el-dialog
                ...
                <div class="filelist" v-for="info of btInfo.files" :key="info.length + Math.random()">{{info.name}}</div>
...
export default {
    ...
    methods : {
        async addTorrent(e) {
            ...
            console.log(this.btInfo.files)
            this.type = 'bt'
            
            // await this.aria2.addTorrent(btFileBase64)
            // this.$router.push('/')
        },
        handleClose(done) {
            this.$confirm('确认关闭？')

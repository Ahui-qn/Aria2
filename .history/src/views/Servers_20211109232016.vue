...
<template>
    <div class="server">
        <!-- <div style="margin-bottom: 20px">
            <el-button size="small" @click="addTab(editableTabsValue)"> 添加新的服务器 </el-button>
        </div> -->
...
export default {
    data() {
        return {
            ...
            tabIndex: 1, // ID用于删除这个tab  //这个需要从localStorage里面读取，不然每次刷新会有重复的key
        }
    },
    computed: {
        ...mapState(['servers']),
    ...
    mounted() {
        this.tabIndex = Number(JSON.parse(localStorage.getItem('aria2Servers')).at(-1).id)
    },
    methods: {

...
// 读取相同字段的不同值并存入
async function refresh(dataField, aria2) {    // 将获取到的新的设置项分别存入不同的设置字段， //dataField为data里的某个字段
        let latest = await aria2.getGlobalOption()
        for (let key1 in dataField) {
            for (let key2 in latest) {
                if (key1 == key2) {
                    dataField[key1] = latest[key2]
                    console.log(latest[key2], dataField[key1])
                }
            }
        }
    }

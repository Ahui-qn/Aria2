...
export default {
    ...
    computed : {
        ...
        handler() {
                ...
                if (currentParams == 'basic') {
                    return  this.basic
                }else if (currentParams == 'http') {
                    return  this.http
                }else if (currentParams == 'rpc') {
                    return  this.rpc
                }else (currentParams == 'advanced') {
    ...
    // },
    async  mounted() {
    // 将获取到的新的设置项分别存入不同的设置字段， //dataField为data里的某个字段
        let currentParams = this.$route.params.type
        let latest = await this.aria2.getGlobalOption()
        let obj = {}
        console.time('a')
        // debugger
        for (let key1 in this[currentParams]) {
            for (let key2 in latest) {
                if (key1 == key2) {
                    obj[key2] = latest[key2]
                }

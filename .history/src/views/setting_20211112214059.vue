...
export default {
    ...
    data() {
        return {
            ...
            type: {}     // 分别为上面四个字段，在不同的route.params下值不同
        }
    },
    computed : {
        ...mapState(['aria2']),
    ...
    watch: {
        $route : {   // 当$route的params变化时触发返回不同的data字段传给子组件渲染不同的设置页面
            handler() {  
    ...
    async  mounted() {
        //将获取到的新的设置项分别存入不同的设置字段  
        this.basic = await this.refresh(this.basic)
        this.http = await this.refresh(this.http)
        this.rpc = await this.refresh(this.rpc)
        this.advanced = await this.refresh(this.advanced)
        
        console.log('父组件 basic',this.basic)    
        console.log('父组件 http',this.http)
        console.log('父组件 rpc',this.rpc)
        console.log('父组件 advanced',this.advanced)
...
debugger
    ...
    methods : {
        async  refresh(type) {    // 将获取到的新的设置项分别存入不同的设置字段， //type为data里的某个字段
            let latest = await this.aria2.getGlobalOption()
            let obj = {}
            console.time('a')
        ...
        //    let val =  valueTransfrom(value)
        //    console.log(val)
        //     await this.aria2.changeGlobalOption(val)
        },
    }
    // 
}

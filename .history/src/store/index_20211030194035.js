...
export default new Vuex.Store({
    ...
    mutations: {
        ...
        selectAll (state) {    // 全部选择函数
            ...
            if (state.selected.length == state.tasks.length && state.selected.length == state.completeds.length) {
                state.selected = []  // 如果两个为空则清空数组
            }else if (state.tasks.length == state.task.length){

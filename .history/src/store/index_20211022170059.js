...
export default new Vuex.Store({
    ...
    mutations: {
        ...
        getProgress(state, task) {   // 当前下载进度，%显示
            if (task.completedLength == '0') {  // ???/等于000
                task.progress = '0%'
            }else {
                task.progress = (task.completedLength / task.totalLength * 100).toFixed(2) + '%'
            }
        },
        getLinkName(state, task) {
            if (task.files?.[0].path) {  // 如果存在此文件
                task.name = task.files[0].path.split('/').at(-1) // path存在是一定有文件名的
            }else {
                task.name =  task.files?.[0].uris?.[0].uri.split('/').at(-1) ?? '未知'
            }
        },
        getDownloadSpeed(state, task) {   // 当前下载速度
            if (task.status == 'paused') {
                ...
                task.speed =  (task.downloadSpeed / 1024 / 1024).toFixed(2) + 'MB/s'
            }else {
                task.speed = (task.downloadSpeed / 1024 ).toFixed(2) + 'KB/s'
            }
        },

...
export default new Vuex.Store({
    ...
    getters: {
        dowloadShowSearch(state) {   // 输入的内容匹配文件名字，搜索
            if (state.search == '') {
                return state.tasks
            }else {
        ...
        completedsShowSearch(state) {   // 输入的内容匹配文件名字，搜索
            console.log('completed')
            if (state.search == '') {
                return state.completeds
            }else {
                return state.completeds.filter(task => {
                    return task.files[0].path.toLowerCase().includes(state.search.toLowerCase())
                })
            }
        },
        discardShowSearch(state) {   // 输入的内容匹配文件名字，搜索
            console.log('deleted')
            if (state.search == '') {
                return state.discard
            }else {
                return state.discard.filter(task => {
                    return task.files[0].path.toLowerCase().includes(state.search.toLowerCase())
                })
            }
        },
        // dowloadShowSearch(state) {   // 输入的内容匹配文件名字，搜索
        //     if (location.href.split('/').at(-1) == 'completed') {
        //         console.log('completed')
        //         if (state.search == '') {
        //             return state.completeds
        //         }else {
        ...
        //             })
        //         }
        //     }else if (location.href.split('/').at(-1) == 'deleted') {
        //         console.log('deleted')
        //         if (state.search == '') {
        //             return state.discard
        //         }else {
    ...
    mutations: {
        sizeSort(state,currentPage) {  // 接收不同页面给不同的数组排序,completeds, discrad,  已完成和discrad没有搜索功能
            if (state.sort == '1') {

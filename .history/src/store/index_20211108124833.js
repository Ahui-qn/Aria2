    ...
    mutations: {
        sizeSort(state,currentPage) {  // 接收不同页面给不同的数组排序,completeds, discrad
            if (currentPage == '/completed') {
                state.completeds = this.commit('quickSort', {arr:state.completeds, val: 'completeds'})
            }else if (currentPage == '/deleted') {
                state.discard = this.commit('quickSort', {arr:state.discard, val: 'discard'})
            }else {
                state.tasks = this.commit('quickSort', {arr:state.tasks, val: 'tasks'})
            }
            state.sort = !state.sort
        },
        quickSort(state, {arr, val}) {   // 快排小到大
            if (arr.length <= 1) {
                return arr
            }
            ...
            // 递归
            state[val] = quickSort(left).concat([pivot], quickSort(right))
        },
        progressSort(state) {

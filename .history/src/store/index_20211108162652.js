    ...
    mutations: {
        sizeSort(state, currentPage) {  // 接收不同页面给不同的数组排序,completeds, discrad
            ...
            if (currentPage == '/completed') {
                state.completeds = quickSort(state.completeds, state.sort, 'size')
            }else if (currentPage == '/deleted') {
                state.discard = quickSort(state.discard, state.sort, 'size')
            }else {
                state.tasks = quickSort(state.tasks, state.sort, 'size')
            }

...
function quickSort(arr, sort) {   // 快排小到大
    ...
    // 递归
    let obj =  quickSort(left).concat([pivot], quickSort(right))
    console.log(obj)
}
...
export default new Vuex.Store({
    ...
    mutations: {
        ...
        editCompleteds(state, {completeds, currentPage}) {
            state.completeds = quickSort(completeds, state.sort)    // 将获取到的completeds赋值到新的completeds上
        },
        editDiscard(state, {discard, currentPage}) {
            state.discard =  quickSort(discard, state.sort) 
        }
    },
